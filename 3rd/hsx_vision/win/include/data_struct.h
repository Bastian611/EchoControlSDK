...
#ifndef PLATFORM_MSTAR_I5
#define PLATFORM_MSTAR_I5 PLATFORM_DEFAULT_VALUE
#endif
...
typedef struct _TIME_MANUAL_CONFIG
{
	char	time[128];	//时间格式为yyyyMMdd HH:mm:ss 
	int		timeZone;  //时区，时区a对应的计算公式为(12+a)*60,其中,a为-12至+12
}TimeManualConfg;
...
#if PLATFORM_MSTAR | PLATFORM_MSTAR_I5	| PLATFORM_HI35XX
...
#if defined(NETSDK) || !PLATFORM_DM365
...
typedef struct
{
	...
	TempHumidityAlarm      temphumidityAlarm;		
}AlarmConfig;
...
typedef struct
{
	...
	int signalLevel;
	int noiseLevel;
#if PLATFORM_MSTAR | PLATFORM_MSTAR_I5
	int channelnum;
#endif
}WIFI_AP_INFO;
